...
local ResourceLoader = require("core.resource_loader")

local gameResources = ResourceLoader("data")
...
local sounds = {
    rainSfx = love.audio.newSource("data/sfx/rain-sfx.ogg", "stream"),
    windSfx = love.audio.newSource("data/sfx/wind-sfx.ogg", "stream"),
    mainMusic = love.audio.newSource("data/music/main-music.ogg", "stream"),
    bushSfx = love.audio.newSource("data/sfx/bush-sfx.wav", "static"),
    prohibitedSfx = love.audio.newSource("data/sfx/prohibited.ogg", "static")
}
...
function love.load()
    ...
    treePlaceholder = {
        x = 0,
        y = 10, --groundObjects[1].y - (treeHeight * 0.2),
        width = 0.2,
        height = 0.2,
        color = {0, 1, 0, 0.0}
    }
...
function love.update(dt)
   ...
   -- UI.setTextByName("woodsText", trees.woodAmount)
    ...
    menuUI.text["water"].text = "Água: " .. fastmath.roundNumber(stats.waterAmount, 0.01) .. "L"
...
function plantPlant() 
    ...
    local tree = {
        x = treePlaceholder.x,
        y = 0, --groundObjects[1].y - (treeHeight * 0.2),
        width = 0.2,
        height = 0.2,
        selected = false,
        health = 1,
...
function love.draw()
    ...
    love.graphics.draw(starsSprite, 0, 0, 0, 1, 1)

    love.graphics.setShader()
    love.graphics.setColor(1, 1, 1, 0.5)
    love.graphics.draw(starsSprite, starsWidth, 0, 0, 1, 1)

    love.graphics.setShader()
    love.graphics.setColor(1, 1, 1, 0.5)
    love.graphics.draw(starsSprite, starsWidth*2, 0, 0, 1, 1)

    love.graphics.setColor(1, 1, 0, 1)
    love.graphics.print("Screen: " .. width .. " x " .. height, 0, 80) --waterAmount
